...
def print_results(results_dic, results_stats_dic, model,
    ...
    """
    if model in {'vgg', 'alexnet', 'resnet'}:
        print(f"The CNN model architecture being used is : {model}")
    else:
        print(f"The CNN model architecture is not recognized ! choose one of the following: vgg, alexnet, or resnet")
    # print the counts : the same values whatever the model is
    print("These are the statistics of the data used:")
    ...
    for key in results_stats_dic:
        if key.startswith('pct_'):
            print(f"{key:25}: {results_stats_dic[key]:.2f}%")
    ...
    if print_incorrect_dogs and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images']):
        ...
        for key in results_dic:
            if sum(results_dic[key][3:]) == 1:
                print("The pet image of {:>26} was incorrectly classified by the model as {:>30} from file {}".format(results_dic[key][0],results_dic[key][1], key))
    ...
    if print_incorrect_breed and (results_stats_dic['n_correct_breed'] != results_stats_dic['n_correct_dogs']):
        ...
        for key in results_dic:
            if (sum(results_dic[key][3:]) == 2) and (results_dic[key][2] == 0):
                print("The dog image of {:>26} was incorrectly classified by the model as {:>30} from file {}".format(results_dic[key][0], results_dic[key][1], key))