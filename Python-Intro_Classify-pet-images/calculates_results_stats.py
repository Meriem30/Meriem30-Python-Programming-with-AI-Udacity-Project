...
def calculates_results_stats(results_dic):
    ...
    # this function
    results_stats_dic = {}
    # the number of all image is the number of files in results_dict
    n_images = len(results_dic)
    # initialize other count variables to 0
    n_dogs_img = 0
    n_match = 0
    n_correct_dogs = 0
    n_correct_notdogs = 0
    n_correct_breed = 0
    # process through each file to increment the counts
    for file_in in results_dic:
        if results_dic[file_in][3] == 1:
            n_dogs_img += 1
            if results_dic[file_in][4] == 1:
                n_correct_dogs += 1
        elif results_dic[file_in][4] == 0:
                n_correct_notdogs += 1
    ...
    pct_match = n_match * 100.0 / n_images if n_images > 0 else 0.0
    pct_correct_dogs = n_correct_dogs * 100.0 / n_dogs_img if n_dogs_img > 0 else 0.0
    pct_correct_breed = n_correct_breed * 100.0 / n_dogs_img if n_dogs_img > 0 else 0.0
    pct_correct_notdogs = n_correct_notdogs * 100.0 / n_notdogs_img if n_dogs_img > 0 else 0.0

    # add the calculated results to the dict
